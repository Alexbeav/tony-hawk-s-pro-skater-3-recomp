...
#
# Writes: dist/tony-hawk-s-pro-skater-3-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix tony-hawk-s-pro-skater-3-recomp \
  --exe-name Tony_Hawks_Pro_Skater_3_Recompiled \
  --display-name "Tony Hawk's Pro Skater 3 Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Tony Hawk's Pro Skater 3 disc" \
